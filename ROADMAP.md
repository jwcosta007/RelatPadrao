...
> Detalhamento técnico das decisões em aberto: `SDD/RegrasRelatPadrao.md` §7.1.

---

## Próxima iteração (pós-piloto AB)

- **Aba `check` — fórmulas de validação:** soma KPI vs cascata N3; `_sem_mapa = 0`; contador `f_Erros` (vermelho se > 0); bate colisão Realizado×Projeção; bate DFC caixa; BU duplo-check. Design em aberto — ver `SDD/RegrasRelatPadrao.md` §7.1.
...
- **Frente Projeções/Forecast (§10):** aba O×R com seletores duplos, gráfico FP&A 3 séries, bate colisão na `check`. Design FECHADO em `SDD/RegrasRelatPadrao.md` §10; implementação pendente.
